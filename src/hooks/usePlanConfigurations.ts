...
const STATIC_PLANS: PlanConfiguration[] = [
  {
    ...
    price_monthly: 0,
    features: ['13 tokens/month', '5 TTS requests/month', 'Basic support', '1 project'],
  ...
  },
  {
    ...
    price_monthly: 9,
    features: ['200 tokens/month', '100 TTS requests/month', 'Priority support', 'File upload', '5 projects'],
  ...
  },
  {
    ...
    price_monthly: 19,
    features: ['500 tokens/month', 'Unlimited TTS', 'Priority support', 'Advanced features', '10 projects'],
    ...
    const fetchPlans = async () => {
      try {
        ...
        const transformedPlans: PlanConfiguration[] = data.map(plan => {
          ...
          const limits = {
            tokens: planLimits && typeof planLimits === 'object' ? Number(planLimits.tokens) || 0 : 0,
