-- Fix the location column type issue and sync users
...
SELECT 
    ...
    true as is_synced,
    ('{"country": "US", "detected": "auto", "timestamp": "' || now() || '"}')::jsonb as location
FROM auth.users au
WHERE NOT EXISTS (
...
-- Add function to automatically sync new users with proper JSONB casting
...
-- Create trigger for auto-sync (replace if exists)
...
SET location = ('{"country": "US", "detected": "auto", "timestamp": "' || now() || '"}')::jsonb
