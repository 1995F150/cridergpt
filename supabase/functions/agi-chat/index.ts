...
async function executeTool(
  ...
  switch (toolName) {
    ...
    case "create_mod_zip": {
      try {
        const modName = args.mod_name || "CriderGPT_Mod";
        const response = await fetch(`${SUPABASE_URL}/functions/v1/process-mod-zip`, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            template_vars: {
              mod_name: modName,
              mod_desc: args.mod_description || "Generated by CriderGPT",
            },
            files: args.files || [],
            user_id: userId,
          }),
        ...
        const fileList = (data.file_list || []).map((f: string) => `  • ${f}`).join("\n");
        return {
          result: `✅ **Mod ZIP created: ${modName}**\n📥 **[Download Link](${data.download_url})**\n\nFiles included (${data.files_count}):\n${fileList}\n\n_Link valid for 1 hour._`,
          status_emoji: "📦",
          status_text: `Mod ZIP created: ${modName}`,
        };
      } catch (e: any) {
        return { result: `Failed to create mod ZIP: ${e.message}`, status_emoji: "❌", status_text: "Mod ZIP failed" };
      }
    }
